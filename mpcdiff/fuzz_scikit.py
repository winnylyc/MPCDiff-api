...
class fuzzer_scikit():
    def __init__(self, args):
        ...
        if args.dataset == 'MNIST':
            ...
            self.mutation_th = 10
        elif args.dataset == 'Credit' or args.dataset == 'Bank_sci':
    ...
    def initialize(self, model, testloader, args, plain_net=None):
        ...
        for data in testloader:
            ...
            if return_dict['label'] != b and b == labels:
                self.AEs.append(copy.deepcopy(images))
                if not args.test_only:
                    saving(images, len(self.AEs), self.args, original=False)
            else:
                current_err = np.linalg.norm(a - return_dict['output'])
                self.pool.append(copy.deepcopy(images))
                self.errs.append(copy.deepcopy(current_err))
                self.true_labels.append(copy.deepcopy(labels))
    ...
    def mutation(self, seed_X, index):
        ...
        return self.projection(seed_X + np.random.normal(0, self.mutation_scale, size=seed_X.size))

    def projection(self, inputs, predefined_min=-0.4242, predefined_max=2.8214):
        if self.args.dataset == 'Credit' or self.args.dataset == 'Bank_sci':
            return np.clip(inputs, a_min=0.0, a_max=1.0)
        ...
        while mutation_num < 15000 and len(self.pool) > 0:
            ...
            if b != return_dict['label'] and b == true_label:
                ...
                with open(file_name, 'a') as txt_file:
                    ...
                    txt_file.write('label from plaintext: %d, label from mpc: %d\n'%(b, return_dict['label']))
            else:
                new_err = np.linalg.norm(a - return_dict['output'])
...
def saving(image, index, args, original=False):
    ...
    if args.dataset == 'Credit' or args.dataset == 'Bank_sci':
...
if __name__ == '__main__':
    ...
    if args.dataset == 'MNIST':
        ...
        # X_train, X_test, y_train, y_test = train_test_split(data_list, label_list, test_size=0.3, random_state=1)
